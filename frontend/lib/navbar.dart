...
class HomeNavController extends InheritedWidget {
  final void Function(int index) goToTab;
  final void Function(double sleepHours) openRescue;
  final Future<bool> Function(int fromIndex) tryOpenRescueTab;

  const HomeNavController({
    super.key,
    required this.goToTab,
    required this.openRescue,
    required this.tryOpenRescueTab,
    required super.child,
  });

  static HomeNavController? of(BuildContext context) {
    return context.dependOnInheritedWidgetOfExactType<HomeNavController>();
  }

  @override
  bool updateShouldNotify(HomeNavController oldWidget) => false;
}
...
class _HomeViewState extends State<_HomeView> {
  ...
  final List<Widget> _pages = const <Widget>[
    ...
    SchedulePage(),
    const RescueModePage(sleepHours: 0),
     SleepHistoryPage(),
  ...
  @override
  Widget build(BuildContext context) {
    Future<bool> tryOpenRescueTab(int fromIndex) async {
      setState(() => _selectedIndex = 2);
      rescuePlanService.reloadIfNeeded();
      return true;
    }

    void handleTabChange(int i) {
      if (i == 2) {
        tryOpenRescueTab(_selectedIndex);
        return;
      }
    ...
    void openRescueWithHours(double sleepHours) async {
      final opened = await tryOpenRescueTab(_selectedIndex);
      if (!opened) return;
      rescuePlanService.setPreferredSleepHours(sleepHours);
      rescuePlanService.reloadIfNeeded();
    }

    return Scaffold(
      backgroundColor: AppColors.scaffoldBg,
      body: HomeNavController(
        goToTab: handleTabChange,
        openRescue: openRescueWithHours,
        tryOpenRescueTab: tryOpenRescueTab,
        child: IndexedStack(
