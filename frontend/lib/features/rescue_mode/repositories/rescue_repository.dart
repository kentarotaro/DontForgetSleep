...
class RescueRepository {
  ...
  Future<RescuePlan> getRescuePlan(double sleepHours) async {
    ...
    // 7. Check if there is an existing rescue session for today in Firestore
    ...
    try {
      ...
      if (sessionsSnapshot.docs.isNotEmpty) {
        final docData = sessionsSnapshot.docs.first.data();
        final geminiResponse = docData['geminiResponse'] as Map<String, dynamic>?;
