...
RescuePlan parseRescuePlan({
  ...
  final aiPayload = _unwrapAiPayload(data);
  final suggestion = aiPayload['sleepWindowSuggestion'] as Map<String, dynamic>?;
  final recommendedBedtime = suggestion?['recommendedBedtime']?.toString() ?? '22:00';

  final caffeineAdvice = aiPayload['caffeineAdvice'];
  final caffeineAdviceText = caffeineAdvice is Map
      ? caffeineAdvice['caffeineCutoffTime']?.toString() ?? caffeineAdvice['tips']?.toString() ?? ''
      : caffeineAdvice?.toString() ?? '';
  final caffeineStop = _extractTimeFromText(caffeineAdviceText) ?? '14:00';

  final checklistItems = _extractChecklistItems(aiPayload);
  ...
  final subtitle = type == RescueType.underslept
      ? "Below your target! Here’s your 24h recovery plan: "
