...
class FirestoreSleepHistoryStore implements SleepHistoryStore {
  ...
  Future<void> addSleepEntry(SleepEntry entry) async {
    ...
    // 1. Update local list first (Optimistic update)
    ...
    // 2. Perform Firestore write in the background
    docRef.set({
      'userId': user.uid,
      'date': dateStr,
      'bedtime': entry.startTime.toLocal().toIso8601String(),
      'wakeTime': entry.endTime.toLocal().toIso8601String(),
      ...
      // ignore: avoid_print
      print('Error saving sleep log to Firestore: $e');
    });
  }
  ...
  Future<void> updateSleepEntry(SleepEntry entry) async {
    ...
    final dateStr = '${entry.endTime.year}-${entry.endTime.month.toString().padLeft(2, '0')}-${entry.endTime.day.toString().padLeft(2, '0')}';
    final docId = entry.id.startsWith('sleep_') ? entry.id : 'sleep_${user.uid}_$dateStr';
    final docRef = FirebaseFirestore.instance.collection('sleepLogs').doc(docId);
    ...
    docRef.set({
      ...
      // ignore: avoid_print
      print('Error updating sleep log in Firestore: $e');
    });
  }
}
