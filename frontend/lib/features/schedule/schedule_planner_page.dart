...
class _SchedulePlannerPageState extends State<SchedulePlannerPage> {
  ...
  Future<void> _loadScheduleFromFirestore() async {
    ...
    try {
      // 1. Fetch commitments and goals from Firestore
  ...
  Future<void> _detectAndTriggerNapNotification() async {
    ...
    final isLatestUnderslept = (latestEntry.duration.inMinutes / 60.0) < targetHours;

    final totalDuration = nightSleeps.take(7).fold<Duration>(Duration.zero, (sum, e) => sum + e.duration);
    final avgHours = nightSleeps.isNotEmpty ? (totalDuration.inMinutes / nightSleeps.length) / 60.0 : 0.0;
    ...
    if (maxGap >= 0.5) { // 30 minutes minimum
      ...
      final timeStr = '${startHour.toString().padLeft(2, '0')}:${startMinute.toString().padLeft(2, '0')}';
      ...
      if (todayIndex < dates.length) {
        ...
        await notificationService.scheduleNapClosingNotification(napStartTime: napStartDateTime);
      }
    }
  }
  ...
  List<ScheduleBlock> _generateSleepBlocks() {
    ...
    if (bed > wake) {
      ...
      blocks.add(ScheduleBlock(name: 'Sleep', start: 0.0, end: wake, type: 'sleep'));
      blocks.add(ScheduleBlock(name: 'Sleep', start: bed, end: 24.0, type: 'sleep'));
    } else if (bed < wake) {
      // Daytime sleep (does not cross midnight)
      blocks.add(ScheduleBlock(name: 'Sleep', start: bed, end: wake, type: 'sleep'));
    }
    return blocks;
  }
  ...
  Map<String, List<ScheduleBlock>> _scheduleForDisplay() {
    ...
    for (final day in _days) {
      displaySchedule[day] = List<ScheduleBlock>.from(_schedule[day] ?? const []);
    }
  ...
  void _handleBlockTap(ScheduleBlock block, String day) {
    ...
    if (block.type == 'sleep') {
      ...
      descriptionText = 'This is your protected sleep floor window to prevent sleep debt.';
      ...
      descriptionText = 'This event is synchronized from your Google Calendar. You can edit it directly in Google Calendar.';
    } else {
      blockTypeLabel = 'AI Scheduled Goal';
      descriptionText = 'This goal is automatically scheduled by the AI planner to optimize your energy levels.';
    }
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            Text(
              ...
              style: TextStyle(
                color: block.type == 'sleep'
                    ? AppColors.teal
                    : (block.type == 'calendar' ? AppColors.bluePrimary : AppColors.purple400),
                fontWeight: FontWeight.bold,
                fontSize: 13,
              ),
            ),
            ...
            Text(
              descriptionText,
              style: const TextStyle(color: AppColors.neutral_400, fontSize: 13, height: 1.4),
            ),
          ],
        ),
            ...
            : SchedulePlannerStepTwo(
                ...
                onComplete: () async {
                  ...
                  if (uid != null) {
                    ...
                    await sleepPreferencesService.completeSchedule(uid, advice: _aiAdvice);
                  }
                },
