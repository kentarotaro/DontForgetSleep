...
import 'package:dont_forget_sleep/services/daily_insight_service.dart';
...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    dailyInsightService.fetchTodayInsight();
  }
  ...
  Widget build(BuildContext context) {
    return AnimatedBuilder(
      ...
      builder: (context, _) {
        ...
        final insight = dailyInsightService.insight;
        final insightErrorCode = dailyInsightService.errorCode;
        final isInsightLoading = dailyInsightService.isLoading;
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  showSubmitted
                      ? _buildSubmittedCard(displayedEntry, targetFloorHours)
            ...
            child: ElevatedButton(
              onPressed: _isSubmittingCheckIn || _hasCheckedInToday(entries)
                  ...
                  : () {
                      ...
                      try {
                        final parts = sleepPreferencesService.preferredWakeTime.split(':');
                      ...
                      final entry = SleepEntry(
                        ...
                        startTime: wakeTime.subtract(duration),
                        endTime: wakeTime,
                      ...
                      // 1. Optimistic UI: Transition to the submitted state immediately
                      setState(() {
                        isSubmitted = true;
                      ...
                      // 2. Perform write and service operations asynchronously in the background
                      Future(() async {
                        try {
                          ...
                          rescuePlanService.markNeedsRefresh();
                          dailyInsightService.fetchTodayInsight();
                        } catch (e) {
                          // ignore: avoid_print
                      ...
                      // 3. Immediately switch/redirect the user to the Rescue Mode page
                      final navController = HomeNavController.of(context);
                      if (navController != null) {
                        navController.openRescue(sleepHours);
                      }
                    },
        ...
        Expanded(
          child: GestureDetector(
            onTap: () {
              ...
              final navController = HomeNavController.of(context);
              if (navController != null) {
                navController.openRescue(hours);
                return;
              }
  ...
  Widget _buildInsightCard({
    required DailyInsightData? insight,
    required bool isLoading,
    required String? errorCode,
  }) {
    String? helper;
    if (errorCode == 'INSUFFICIENT_SLEEP_DATA') {
      helper = 'Log a few more nights to unlock daily AI insight.';
    } else if (errorCode != null && errorCode != 'EMPTY_DATA') {
      helper = 'Insight is temporarily unavailable.';
    }
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(16),
        border: Border.all(color: const Color(0xFF1E3B57), width: 1),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          ...
          if (isLoading)
            ...
            if (insight.recommendation.isNotEmpty) ...[
              ...
              Text(
                ...
                style: const TextStyle(color: Color(0xFFB7E6BE), fontSize: 12),
              ),
            ],
          ] else
            Text(
              helper ?? 'No insight yet for today.',
              style: const TextStyle(color: Colors.white70, fontSize: 12),
            ),
        ],
      ),
