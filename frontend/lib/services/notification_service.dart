...
class NotificationService {
  ...
  Future<void> showNapNotification(String duration, String time) async {
    ...
    final title = durationMinutes >= 90
        ? '💤 Recovery Nap Window'
        : durationMinutes >= 45
            ? '💤 Strong Nap Window'
            : '💤 Quick Reset Nap Window';

    final body = durationMinutes >= 90
        ? 'You have a long gap at $time ($duration) for deeper recovery.'
        : durationMinutes >= 45
            ? 'You have a $duration gap at $time. A medium nap can boost alertness.'
            : 'You have a short $duration break at $time. A quick nap can still help.';
  ...
  int _parseDurationToMinutes(String duration) {
    final normalized = duration.toLowerCase().trim();
    final hourMatch = RegExp(r'(\d+(?:\.\d+)?)h').firstMatch(normalized);
    final minuteMatch = RegExp(r'(\d+)m').firstMatch(normalized);

    var minutes = 0;
    if (hourMatch != null) {
      final parsedHours = double.tryParse(hourMatch.group(1) ?? '0') ?? 0;
      minutes += (parsedHours * 60).round();
    }
    if (minuteMatch != null) {
